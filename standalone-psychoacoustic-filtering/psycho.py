...
if __name__ == "__main__":
    ...
    # input
    original_file = Path("/root/sample.wav")

    # compute hearing thresholdsd
    print(f"[+] Get thresholds")
    Psycho.calc_thresholds(original_file)
    
    # filter for different levels of margins for hearing threshs in dB
    ...
    for phi in [-4, -2, -1, 0, 1, 2, 4, 8, 16, 32]:
        out_file = Path(f"/root/filtered_{phi}dB.wav")
        print(f"[+] Convert {original_file.name} into {out_file.name}")
        Psycho(phi).convert_wav(original_file, threshs_file, out_file)