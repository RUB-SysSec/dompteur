...
if [ $stage -le -4 ]; then
  ...
  steps/nnet2/get_lda.sh $lda_opts "${extra_opts[@]}"  --do_lda "false" --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
#egs_opts=(cmvn_opts="--norm-means false")
#echo $egs_opts
...
if [ $stage -le -2 ]; then
  ...
  overlap=$tot_input_dim
  tot_input_dim_double=$[$feat_dim+$overlap]
  tot_input_dim=$[$tot_input_dim_double/2]

  startidx=$LOW_FREQUENCY_IDX
  endidx=$HIGH_FREQUENCY_IDX

  tot_input_dim_filter=$[$endidx-$startidx+1]
  lda_dim=$[$tot_input_dim_filter*9]
...
ScaleComponent dim=$tot_input_dim scale=1
OverlapComponent input-dim=$tot_input_dim overlap=$overlap
WindowComponent input-dim=$tot_input_dim_double win-type=3
FftComponent dim=$tot_input_dim_double fft-dim=$tot_input_dim_double reorder=false fft-keep-dim=0
AbsComponent input-dim=$tot_input_dim_double
FilterComponent input-dim=$tot_input_dim start-idx=$startidx end-idx=$endidx
LogComponent input-dim=$tot_input_dim_filter
SpliceComponent input-dim=$tot_input_dim_filter left-context=$splice_width right-context=$splice_width const-component-dim=$ivector_dim
