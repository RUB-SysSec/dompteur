...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("rir-layer", &rir_layer, "Defines Room Impulse Response Layer."); 
...
/*#if HAVE_CUDA==1
  ...
  std::string data(rxfilename);
    ...
    AmNnet am_nnet;
    {
      ...
      if(rir_layer.compare("no") != 0) {
        Input rir(rir_layer, &binary);
        am_nnet.AddComponent(rir.Stream(), binary);
      }
    }
