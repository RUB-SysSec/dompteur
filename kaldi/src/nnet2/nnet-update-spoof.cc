  ...
  for (int32 c = nnet_.NumComponents()-1;c >= 0; c--) {
    ...
    if(component.Type().compare("FftComponent") == 0 && thresh != -1)
    {
      ...
      if(original_magn.NumRows() != 0) {
        ...
        // int above = 0;
        // int below = 0;
        // for (int c = 0; c < scale_factors.NumCols(); c++) {
        //   for (int r = 0; r < scale_factors.NumRows(); r++) {
        //     if(scale_factors(r,c) >= 0){
        //       above += 1;
        //     } else {
        //       below += 1;
        //     }
        //   }
        // }
        // KALDI_LOG << "above " << above << " below " << below;

        scale_factors.Scale(-1.0/(66.2266+95.0));

    //     KALDI_LOG << "BEFORE " << scale_factors(rr, cc);
		// for(int c = 0; c < scale_factors.NumCols(); c++) {
		// 	for(int r = 0; r <  scale_factors.NumRows(); r++) {
		// 		if(scale_factors(r,c) < 0)
		// 			scale_factors(r,c) = 1;
		// 		else
		// 			scale_factors(r,c) = scale_factors(r,c) * 10;
				
		// 	}
		// }
		// KALDI_LOG << "AFTER " << scale_factors(rr, cc);
    ...
    // double log_min_in = 1000000;
    // double log_min_out = 1000000;
    // double log_max_in = std::numeric_limits<BaseFloat>::epsilon();
    // double log_max_out = std::numeric_limits<BaseFloat>::epsilon();
    // double log_max_grad_in = std::numeric_limits<BaseFloat>::epsilon();
    // double log_max_grad_out = std::numeric_limits<BaseFloat>::epsilon();
    // for(int c = 0; c < output_deriv.NumCols(); c++) {
    //   for(int r = 0; r <  output_deriv.NumRows(); r++) {

    //     if(input(r,c) < log_min_in) 
    //       log_min_in = input(r,c);

    //     if(output(r,c) < log_min_out) 
    //       log_min_out = output(r,c);

    //     if(input(r,c) > log_max_in) 
    //       log_max_in = input(r,c);

    //     if(output(r,c) > log_max_out) 
    //       log_max_out = output(r,c);

    //     if(output_deriv(r,c) > log_max_grad_out) 
    //       log_max_grad_out = output_deriv(r,c);

    //     if(input_deriv(r,c) > log_max_grad_in) 
    //       log_max_grad_in = input_deriv(r,c);
        
    //   } 
    // } 

    // KALDI_LOG << "[+] " << component.Type();
    // KALDI_LOG << "    Min right " <<  log_min_out;
    // KALDI_LOG << "    Min left  " <<  log_min_in;
    // KALDI_LOG << "    Max right " <<  log_max_out;
    // KALDI_LOG << "    Max left  " <<  log_max_in;
    // KALDI_LOG << "    Max grad right " <<  log_max_grad_out;
    // KALDI_LOG << "    Max grad left  " <<  log_max_grad_in;
