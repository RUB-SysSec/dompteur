...
void Nnet::AddComponent(std::istream &is, bool binary) {

  std::vector<Component*> components_old;
  components_old = components_;
  int32 num_components = components_.size() + 1;
  components_.resize(num_components);

  components_[0] = Component::ReadNew(is, binary);

  for (int32 c = 0; c < num_components - 1; c++)
    components_[c + 1] = components_old[c];
}
