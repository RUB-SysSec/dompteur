...
Component* Component::NewComponentOfType(const std::string &component_type) {
  ...
  } else if (component_type == "WindowComponent") {
    ans = new WindowComponent();
  ...
  } else if (component_type == "FftComponent") {
    ans = new FftComponent();
  ...
  } else if (component_type == "ConvRIRComponent") {
    ans = new ConvRIRComponent();
  } else if (component_type == "OverlapComponent") {
    ans = new OverlapComponent();
  ...
  }else if (component_type == "FilterComponent") {
    ans = new FilterComponent();
  }else if (component_type == "PytorchComponent") {
    ans = new PytorchComponent();
  }else if (component_type == "AbsComponent") { 
    ans = new AbsComponent();
  }
  return ans;
}
...
void WindowComponent::Init(int32 input_dim, int32 type) {
  input_dim_ = input_dim;
  type_ = type;
}

void WindowComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  int32 input_dim = 0;
  int32 type = 0;
  bool ok = ParseFromString("input-dim", &args, &input_dim);
  ok = ParseFromString("win-type", &args, &type) && ok;
...
void WindowComponent::Propagate(const ChunkInfo &in_info,
                               const ChunkInfo &out_info,
                               const CuMatrixBase<BaseFloat> &in,
                               CuMatrixBase<BaseFloat> *out) const  {
  // corresponds to a window function, e.g. Hamming window
  // x_w = x * w
  CuVector<BaseFloat> window(input_dim_);
  if(type_ == 1) { // hamming
    double a = M_2PI / (input_dim_-1);
    for (int32 i = 0; i < input_dim_; i++) {
      double i_fl = static_cast<double>(i);
      window(i) = 0.54 - 0.46*cos(a * i_fl);
    }
  }else if(type_ == 2) { // rect
    window.Set(1);
  }else if(type_ == 3) { // Nuttall
    double a = M_2PI / (input_dim_-1);
    for (int32 i = 0; i < input_dim_; i++) {
      double i_fl = static_cast<double>(i);
      window(i) = 0.3635819 -  0.4891775*cos(a * i_fl) + 0.1365995*cos(2 * a * i_fl) - 0.0106411*cos(3 * a * i_fl);
    }
  }
  ...
  out->CopyFromMat(in);
  out->MulColsVec(window);
}

void WindowComponent::Backprop(const ChunkInfo &,  // in_info,
                              const ChunkInfo &,  // out_info,
                              const CuMatrixBase<BaseFloat> &in_value,
                              const CuMatrixBase<BaseFloat> &out_value,
                              const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  if(type_ == 1) { // hamming
    ...
    for (int32 i = 0; i < input_dim_; i++) {
      double i_fl = static_cast<double>(i);
      //window(i) = 0.35875 - 0.48829*cos(a * i_fl) + 0.14128*cos(2 * a * i_fl) - 0.01168*cos(3 * a * i_fl);
      window(i) = 0.3635819 -  0.4891775*cos(a * i_fl) + 0.1365995*cos(2 * a * i_fl) - 0.0106411*cos(3 * a * i_fl);
    }
  }

  in_deriv->Resize(out_deriv.NumRows(), out_deriv.NumCols());
  in_deriv->CopyRowsFromVec(window);

  in_deriv->MulElements(out_deriv);
}

void WindowComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<WindowComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "<WindowType>");
  ReadBasicType(is, binary, &type_);
  ExpectToken(is, binary, "</WindowComponent>");
}

void WindowComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<WindowComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "<WindowType>");
  WriteBasicType(os, binary, type_);
  WriteToken(os, binary, "</WindowComponent>");
}

std::string WindowComponent::Info() const {
  std::stringstream stream;
  stream << Type() << ", input-dim = " << input_dim_;//<< ", win-type = " << type_;
  return stream.str();
}



void LogComponent::Init(int32 input_dim) {
  input_dim_ = input_dim;
}

void LogComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  int32 input_dim = 0;
  bool ok = ParseFromString("input-dim", &args, &input_dim);
  if (!ok || !args.empty())
...
void LogComponent::Propagate(const ChunkInfo &in_info,
                               const ChunkInfo &out_info,
                               const CuMatrixBase<BaseFloat> &in,
                               CuMatrixBase<BaseFloat> *out) const  {
  // Apply log function to each element of the output...
  // the log function may be written as log(x),
  ...
  int log_clamp = atoi( std::getenv("LOG_CLAMP") );
  if (log_clamp != 0){
    KALDI_LOG << "[+] Log clamp " << log_clamp;
    ...
    //   } 
    // } 
    // KALDI_LOG << "[+] LogComponent::Propagate";
    // KALDI_LOG << "    Before Min " <<  min_val;
    // KALDI_LOG << "    Before Max " <<  max_val;
    // KALDI_LOG << "    Before Mean " <<  (mean / N);

    
      for(int c = 0; c < out->NumCols(); c++) {
        for(int r = 0; r <  out->NumRows(); r++) {
          if( (*out)(r,c) < log_clamp )
            (*out)(r,c) = log_clamp;
        }
      }
    
  //   min_val = 10000000;
  //   max_val = std::numeric_limits<BaseFloat>::epsilon();
  //   mean = 0;
  //   N = 0;
  //   for(int c = 0; c < out->NumCols(); c++) {
  //     for(int r = 0; r <  out->NumRows(); r++) {
  ...
  //     } 
  //   } 
  //   KALDI_LOG << "    After Min " <<  min_val;
  //   KALDI_LOG << "    After Max " <<  max_val;
  //   KALDI_LOG << "    After Mean " <<  (mean / N);
...
void LogComponent::Backprop(const ChunkInfo &,  // in_info,
                              const ChunkInfo &,  // out_info,
                              const CuMatrixBase<BaseFloat> &in_value,
                              const CuMatrixBase<BaseFloat> &out_value,
                              const CuMatrixBase<BaseFloat> &out_deriv,
...
*/
  ...
  if (log_clamp != 0){
    KALDI_LOG << "[+] Log clamp " << log_clamp;
    CuMatrix<BaseFloat> tmp(in_value);

    // double min_val = 10000000;
    // double max_val = std::numeric_limits<BaseFloat>::epsilon();
    // for(int c = 0; c < tmp.NumCols(); c++) {
    //   for(int r = 0; r <  tmp.NumRows(); r++) {
    //     if(tmp(r,c) < min_val) 
    //       min_val = tmp(r,c);
    //     if(tmp(r,c) > max_val) 
    //       max_val = tmp(r,c);
    //   } 
    // } 
    // KALDI_LOG << "[+] LogComponent::Backprop :: 1";
    // KALDI_LOG << "    Before Min " <<  min_val;
    // KALDI_LOG << "    Before Max " <<  max_val;

    for(int c = 0; c < tmp.NumCols(); c++) {
      for(int r = 0; r < tmp.NumRows(); r++) {
        if( tmp(r,c) < log_clamp )
          tmp(r,c) = log_clamp;
      }
    }
    ...
    in_deriv->DivElements(tmp);
  } else {
    in_deriv->CopyFromMat(out_deriv);
    in_deriv->DivElements(in_value);
  }

  // const CuMatrixBase<BaseFloat> *tmp;
  // tmp->CopyFromMat(in_value);

}

void LogComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<LogComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "</LogComponent>");
}

void LogComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<LogComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "</LogComponent>");
}
...
void FilterComponent::Init(int32 input_dim, int32 start_idx, int32 end_idx) {
  input_dim_ = input_dim;
  start_idx_ = start_idx;
  end_idx_ = end_idx;
}

void FilterComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  int32 start_idx, end_idx, input_dim = 0;
  bool ok = ParseFromString("input-dim", &args, &input_dim);
  ok = ParseFromString("start-idx", &args, &start_idx) && ok;
  ok = ParseFromString("end-idx", &args, &end_idx) && ok;
  if (!ok || !args.empty())
    KALDI_ERR << "Invalid initializer for layer of type "
              << Type() << ": \"" << orig_args << "\"";
  Init(input_dim, start_idx, end_idx);
}

void FilterComponent::Propagate(const ChunkInfo &in_info,
                                const ChunkInfo &out_info,
                                const CuMatrixBase<BaseFloat> &in,
                                CuMatrixBase<BaseFloat> *out) const {
  in_info.CheckSize(in);
  ...
  CuSubMatrix<BaseFloat> in_mat(in, 0, num_rows, start_idx_, end_idx_ - start_idx_ + 1);
  out->CopyFromMat(in_mat);
  // out->CopyFromMat(in);
}

void FilterComponent::Backprop(const ChunkInfo &,  // in_info,
                              const ChunkInfo &,  // out_info,
                              const CuMatrixBase<BaseFloat> &in_value,
                              const CuMatrixBase<BaseFloat> &out_value,
                              const CuMatrixBase<BaseFloat> &out_deriv,
                              Component *to_update,
                                // may be identical to "this".
                              CuMatrix<BaseFloat> *in_deriv) const  {
  int32 num_rows = out_deriv.NumRows();
  KALDI_LOG << "MIDDLE";
  KALDI_LOG << "in_deriv COLS: " << in_deriv->NumCols() << " in_deriv ROWS: " << in_deriv->NumRows();
  CuSubMatrix<BaseFloat> out_deriv_mat_middle(*in_deriv, 0, num_rows, start_idx_, end_idx_ - start_idx_ + 1);
  out_deriv_mat_middle.CopyFromMat(out_deriv);
  KALDI_LOG << "out_deriv_mat_middle COLS: " << out_deriv_mat_middle.NumCols() << " out_deriv_mat_middle ROWS: " << out_deriv_mat_middle.NumRows();

  if(start_idx_ != 0) {
    KALDI_LOG << "START";
    CuSubMatrix<BaseFloat> out_deriv_mat_start(*in_deriv, 0, num_rows, 0, start_idx_);
    CuMatrix<BaseFloat> zeros_start(num_rows, start_idx_, kSetZero);
    out_deriv_mat_start.CopyFromMat(zeros_start);
    KALDI_LOG << "out_deriv_mat_start COLS: " << out_deriv_mat_start.NumCols() << " out_deriv_mat_start ROWS: " << out_deriv_mat_start.NumRows();
  }

  if(end_idx_ + 1 < in_deriv->NumCols()){
    KALDI_LOG << "END";
    CuSubMatrix<BaseFloat> in_deriv_mat_end(*in_deriv, 0, num_rows, end_idx_ + 1, in_deriv->NumCols() - end_idx_ - 1);
    CuMatrix<BaseFloat> zeros_end(num_rows, in_deriv->NumCols() - end_idx_ - 1, kSetZero);
    in_deriv_mat_end.CopyFromMat(zeros_end);
    KALDI_LOG << "in_deriv_mat_end COLS: " << in_deriv_mat_end.NumCols() << " in_deriv_mat_end1 ROWS: " << in_deriv_mat_end.NumRows();
  }

  // std::string data_file_in = DumpMatrixIntoFilewithSuffix(*in_deriv, "_dev_in.csv");
  // KALDI_LOG << "IN DEV: " << data_file_in;
  // std::string data_file_out = DumpMatrixIntoFile(out_deriv, data_file_in + "_dev_out.csv");
  // KALDI_LOG << "OUT DEV: " << data_file_out;
  // std::string file_out = DumpMatrixIntoFile(in_value, data_file_in + "_in.csv");
  // KALDI_LOG << "OUT: " << file_out;
  // std::string file_in = DumpMatrixIntoFile(out_value, data_file_in + "_out.csv");
  // KALDI_LOG << "IN: " << file_in;
}

void FilterComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<FilterComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "<StartIdx>");
  ReadBasicType(is, binary, &start_idx_);
  ExpectToken(is, binary, "<EndIdx>");
  ReadBasicType(is, binary, &end_idx_);
  ExpectToken(is, binary, "</FilterComponent>");
}

void FilterComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<FilterComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "<StartIdx>");
  WriteBasicType(os, binary, start_idx_);
  WriteToken(os, binary, "<EndIdx>");
  WriteBasicType(os, binary, end_idx_);
  WriteToken(os, binary, "</FilterComponent>");
}

std::string FilterComponent::Info() const {
  std::stringstream stream;
  stream << Type() << ", input-dim = " << input_dim_;
  return stream.str();
}
...
void PytorchComponent::Init(int32 input_dim) {
  input_dim_ = input_dim;
}

void PytorchComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  int32 input_dim = 0;
  bool ok = ParseFromString("input-dim", &args, &input_dim);
  if (!ok || !args.empty())
...
void PytorchComponent::Propagate(const ChunkInfo &in_info,
                               const ChunkInfo &out_info,
                               const CuMatrixBase<BaseFloat> &in,
                               CuMatrixBase<BaseFloat> *out) const  {
  ...
  std::string data_file = DumpMatrixIntoTempFile(in);

  // invoke python
  std::stringstream cmd_line;
  cmd_line << "/usr/bin/python3 /root/kaldi/wsj_recipe/psycho/pytorch_propagate.py --data_file " << data_file << " --threshs_file_id " << getpid();
  KALDI_LOG << cmd_line.str();
  system(cmd_line.str().c_str());

  CuMatrix<BaseFloat> tmp = ReadMatrixFromTempFile(data_file, in.NumRows(), in.NumCols());
  out->CopyFromMat(tmp);

}

void PytorchComponent::Backprop(const ChunkInfo &,  // in_info,
                              const ChunkInfo &,  // out_info,
                              const CuMatrixBase<BaseFloat> &in_value,
                              const CuMatrixBase<BaseFloat> &out_value,
                              const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  std::string data_in_file = DumpMatrixIntoTempFile(in_value);
  std::string data_out_file = DumpMatrixIntoTempFile(out_value);
  std::string grad_file = DumpMatrixIntoTempFile(out_deriv);
  KALDI_LOG << data_in_file;
  KALDI_LOG << data_out_file;
  KALDI_LOG << grad_file;

  // invoke python
  std::stringstream cmd_line;
  cmd_line << "/usr/bin/python3 /root/kaldi/wsj_recipe/psycho/pytorch_backprop.py --data_in_file " << data_in_file << " --data_out_file " << data_out_file << " --grad_file " << grad_file << " --threshs_file_id " << getpid();
  KALDI_LOG << cmd_line.str();
  system(cmd_line.str().c_str());

  std::string cmd_line_rm = "rm " + data_in_file;
  system(cmd_line_rm.c_str());

  cmd_line_rm = "rm " + data_out_file;
  system(cmd_line_rm.c_str());

  CuMatrix<BaseFloat> tmp = ReadMatrixFromTempFile(grad_file, out_deriv.NumRows(), out_deriv.NumCols());
  in_deriv->CopyFromMat(tmp);

}

void PytorchComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<PytorchComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "</PytorchComponent>");
}

void PytorchComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<PytorchComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "</PytorchComponent>");
}

std::string PytorchComponent::Info() const {
  std::stringstream stream;
  stream << Type() << ", input-dim = " << input_dim_;
  return stream.str();
}
...
void AbsComponent::Init(int32 input_dim) {
  input_dim_ = input_dim;
}

void AbsComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  int32 input_dim = 0;
  bool ok = ParseFromString("input-dim", &args, &input_dim);
  if (!ok || !args.empty())
...
void AbsComponent::Propagate(const ChunkInfo &in_info,
                               const ChunkInfo &out_info,
                               const CuMatrixBase<BaseFloat> &in,
                               CuMatrixBase<BaseFloat> *out) const  {
  // Apply log function to each element of the output...
  ...
  CuSubMatrix<BaseFloat> real_mat(in, 0, in.NumRows(), 0, in.NumCols()/2),
                        imag_mat(in, 0, in.NumRows(), in.NumCols()/2, in.NumCols()/2);

  CuMatrix<BaseFloat> real_mat_sq(real_mat);
  CuMatrix<BaseFloat> imag_mat_sq(imag_mat);

  real_mat_sq.MulElements(real_mat);
  imag_mat_sq.MulElements(imag_mat);

  out->CopyFromMat(real_mat_sq);
  out->AddMat(1,imag_mat_sq);
}

void AbsComponent::Backprop(const ChunkInfo &,  // in_info,
                              const ChunkInfo &,  // out_info,
                              const CuMatrixBase<BaseFloat> &in_value,
                              const CuMatrixBase<BaseFloat> &out_value,
                              const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  in_deriv->Resize(out_deriv.NumRows(), out_deriv.NumCols()*2);

  CuSubMatrix<BaseFloat> real_mat(in_value, 0, in_value.NumRows(), 0, in_value.NumCols()/2),
                        imag_mat(in_value, 0, in_value.NumRows(), in_value.NumCols()/2, in_value.NumCols()/2);

   CuSubMatrix<BaseFloat>  in_part1(*in_deriv, 0, in_deriv->NumRows(),
                                 0, in_deriv->NumCols()/2),
                         in_part2(*in_deriv, 0, in_deriv->NumRows(),
                                  in_deriv->NumCols()/2, in_deriv->NumCols()/2);

  in_part1.CopyFromMat(out_deriv);
  in_part2.CopyFromMat(out_deriv);

  in_part1.MulElements(real_mat);
  in_part2.MulElements(imag_mat);

  in_part1.Scale(2);
  in_part2.Scale(2);
}

void AbsComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<AbsComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "</AbsComponent>");
}

void AbsComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<AbsComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "</AbsComponent>");
}

std::string AbsComponent::Info() const {
  std::stringstream stream;
  stream << Type() << ", input-dim = " << input_dim_;
  return stream.str();
}
...
std::string ConvRIRComponent::Info() const {
  std::stringstream stream;
  stream << ", filter_params=" << filter_params_;

  return stream.str();
}

void ConvRIRComponent::Init(int32 input_dim, CuMatrix<BaseFloat> filter_params) {
...
void ConvRIRComponent::InitFromString(std::string args) {
  std::string orig_args(args);

  int32 input_dim;
  CuMatrix<BaseFloat> filter_params;

  ParseFromString("input-dim", &args, &input_dim);
  // TODO define ParseFromString() for CuMatrix<BaseFloat> input, approx line 165 nnet-component.cc
  //ParseFromString("filter_params", &args, &filter_params);

  Init(input_dim, filter_params);
}

int32 ConvRIRComponent::OutputDim() const {
  return input_dim_;
}


void ConvRIRComponent::Propagate(const ChunkInfo &in_info,
                                const ChunkInfo &out_info,
                                const CuMatrixBase<BaseFloat> &in,
                                CuMatrixBase<BaseFloat> *out) const  {
    in_info.CheckSize(in);
    ...
    /*std::string dir_utt = "/home/lea/test";
    ...
    std::string file_in = dir_utt + "/in.csv";

    CuMatrix<BaseFloat> temp_in(in);
    WriteCuMatrixBaseFloat(file_in, temp_in);*/


    const int32 filter_dim = filter_params_.NumCols();  // M
    const int32 batch_size = in.NumCols();              // N 256 (508 ROWS)
    
    if(filter_dim < batch_size) {
        for (int32 index = 1; index < filter_dim; index++) {

            CuMatrix<BaseFloat> patches(in.NumRows(), filter_dim);
            patches.Set(0);

            CuSubMatrix<BaseFloat> data1(in, 0, in.NumRows(),
                                          0, index),
                                    data2(in, 0, in.NumRows() - 1,
                                          in.NumCols() - (filter_dim - index), filter_dim - index),
                                    patch1(patches, 0, patches.NumRows(),
                                          patches.NumCols() - index, index),
                                    patch2(patches, 1, patches.NumRows() - 1,
                                          0, filter_dim - index);


            patch1.CopyFromMat(data1);   
            patch2.CopyFromMat(data2);  

            CuSubMatrix<BaseFloat> patch(patches, 0, in.NumRows(),
                                          0, filter_dim);

            CuSubMatrix<BaseFloat> out_batch(*out, 0, out->NumRows(), index-1 , 1);

            out_batch.AddMatMat(1.0, patch, kNoTrans,
                                    filter_params_, kTrans, 1.0);

        }

        for (int32 index = filter_dim; index <= batch_size; index++) {

            CuMatrix<BaseFloat> patches(in.NumRows(), filter_dim);

            CuSubMatrix<BaseFloat> data(in, 0, in.NumRows(),
                                          index - filter_dim, filter_dim);
            CuSubMatrix<BaseFloat> patch(patches, 0, in.NumRows(), 0, filter_dim);

            patch.CopyFromMat(data);  
        ...
        const int32 num_frames = filter_dim/batch_size;
        const int32 rem = filter_dim - batch_size*num_frames;

        const int32 r = 10;

        for (int32 index = 0; index < batch_size; index++) {

            CuMatrix<BaseFloat> patches(in.NumRows(), filter_dim);
            patches.Set(0);

            //KALDI_LOG << "PRE: " << index;
            // KALDI_LOG << "data ROW: " << 0 << "    " << index + 1;
            //KALDI_LOG << "pre patch COLS: " << patches.NumCols() - index - 1 << "    " << index + 1;

            CuSubMatrix<BaseFloat> data1(in, 0, in.NumRows(),
                                          0, index + 1);
            CuSubMatrix<BaseFloat> patch1(patches, 0, in.NumRows(), 
                                          patches.NumCols() - index - 1, index + 1);
            ...
            /*for(int c = 0; c < 6; c++) {
              ...
              KALDI_LOG << "in-1: r: " << r << "c: " << c << "    " << in(r, c);
            }

            for(int c = 0; c < 6; c++) {

              KALDI_LOG << "patch: r: " << r << "c: " << c+in.NumCols() - index - 1 << "    " << patches(r, c+in.NumCols() - index - 1);
            }
            KALDI_LOG << "----------------------------------------------------------------------------";*/


            int32 end_frame =  num_frames;
            int32 first_frame = 1;
            ...
            for(int32 frame = 0; frame < end_frame; frame ++) {

                //KALDI_LOG << "middle patches COLS: " << patches.NumCols() - (end_frame - frame) * batch_size - index - 1 << "   " << in.NumCols();


                /*KALDI_LOG << "data ROW: " << num_frames-(end_frame-frame) << "    " << in.NumRows()-num_frames;
                KALDI_LOG << "patch ROW: " << num_frames << "    " << in.NumRows()-num_frames;*/

                CuSubMatrix<BaseFloat> data(in, num_frames-end_frame-1, in.NumRows()-(num_frames-frame)+1,
                                              0, in.NumCols());
                CuSubMatrix<BaseFloat> patch(patches, num_frames-frame - 1, in.NumRows()-(num_frames-frame)+1, 
                                              patches.NumCols() - (end_frame - frame) * batch_size - index -1, in.NumCols());
            ...
            int32 offset = rem - index - 1 + (1 - first_frame) * batch_size;


            if(offset != 0) {
                /*KALDI_LOG << "POST: " << offset;

                KALDI_LOG << "patch ROW: " << num_frames + first_frame;*/

                CuSubMatrix<BaseFloat> data2(in, 0, in.NumRows()-first_frame-num_frames,
                                              in.NumCols() - offset, offset);
                CuSubMatrix<BaseFloat> patch2(patches, num_frames + first_frame, in.NumRows()-first_frame - num_frames, 
                                              0, offset);

                patch2.CopyFromMat(data2);
            }

            //KALDI_LOG << "INDEX: " << index;

            CuSubMatrix<BaseFloat> out_batch(*out, 0, out->NumRows(), index , 1);

            out_batch.AddMatMat(1.0, patches, kNoTrans,
                                filter_params_, kTrans, 1.0);


           /* if(index < 10 ) {
                std::string file_patch = dir_utt + "/" + std::to_string(index) + ".csv";

                CuMatrix<BaseFloat> temp_patch(patches);
                WriteCuMatrixBaseFloat(file_patch, temp_patch);
            }*/

            /*int32 r = 0;
            for(int c = 0; c < 6; c++) {
              
                KALDI_LOG << "in: r: " << r << "c: " << c << "    " << in(r, c);
            }

            for(int c = 0; c < 6; c++) {
              
                KALDI_LOG << "patch: r: " << r << "c: " << patches.NumCols()-6+c << "    " << patches(r, patches.NumCols()-6+c);
            }

            for(int c = 0; c < 6; c++) {
              
                KALDI_LOG << "filter_params_: r: " << r << "c: " << filter_params_.NumCols()-6+c << "    " << filter_params_(r, filter_params_.NumCols()-6+c);
            }

            for(int c = 0; c < 6; c++) {

                KALDI_LOG << "out_batch: r: " << r << "c: " << c << "    " << (*out)(r, c);
            }
    ...
    /*std::string file = dir_utt + "/out.csv";

    CuMatrix<BaseFloat> temp(*out);
    WriteCuMatrixBaseFloat(file, temp);*/
    ...
    /*for(int c = 0; c < in.NumCols(); c++) {
        for(int r = 0; r < in.NumRows(); r++) {
            ...
            if((*out)(r, c) != in(r,c))
                KALDI_LOG << "NOT EQUAL: r: " << r << "c: " << c << "    " << in(r,c) << "    " << (*out)(r,c);
        } 
    }
...
void ConvRIRComponent::Backprop(const ChunkInfo &in_info,
                               const ChunkInfo &out_info,
                               const CuMatrixBase<BaseFloat> &in_value,
                               const CuMatrixBase<BaseFloat> &out_value,
                               const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  in_info.CheckSize(out_deriv);
  out_info.CheckSize(*in_deriv);
  KALDI_ASSERT(in_info.NumChunks() == out_info.NumChunks());


  const int32 filter_dim = invers_filter_params_.NumCols();  // M
  const int32 batch_size = out_deriv.NumCols();              // N 256 (508 ROWS)
  
  if(filter_dim < batch_size) {

      for (int32 index = 0; index <= batch_size - filter_dim; index++) {

        CuMatrix<BaseFloat> patches(out_deriv.NumRows(), filter_dim);

        CuSubMatrix<BaseFloat> data(out_deriv, 0, out_deriv.NumRows(),
                                      index, filter_dim);
        CuSubMatrix<BaseFloat> patch(patches, 0, out_deriv.NumRows(), 0, filter_dim);

        patch.CopyFromMat(data);  

        CuSubMatrix<BaseFloat> out_batch(*in_deriv, 0, in_deriv->NumRows(), index, 1);

        out_batch.AddMatMat(1.0, patches, kNoTrans,
                                invers_filter_params_, kTrans, 1.0);
      ...
      for (int32 index = 1; index < filter_dim; index++) {
        ...
        CuSubMatrix<BaseFloat> data1(out_deriv, 0, out_deriv.NumRows(),
                                      out_deriv.NumCols() - (filter_dim - index), filter_dim - index),
                                data2(out_deriv, 1, out_deriv.NumRows() - 1,
                                      0, index),
                                patch1(patches, 0, out_deriv.NumRows(),
                                      0, filter_dim - index),
                                patch2(patches, 0, out_deriv.NumRows() - 1,
                                      patches.NumCols() - index, index);
        ...
        CuSubMatrix<BaseFloat> out_batch(*in_deriv, 0, in_deriv->NumRows(), in_deriv->NumCols() - (filter_dim - index), 1);
        ...
        for (int32 index = 0; index < batch_size; index++) {
            ...
            //KALDI_LOG << "pre patch COLS: " << 0 << "    " << batch_size - index;
            //KALDI_LOG << "pre patch ROWS: " << 0 << "    " << out_deriv.NumRows();

            CuSubMatrix<BaseFloat> data1(out_deriv, 0, out_deriv.NumRows(),
                                          index, batch_size - index);
            CuSubMatrix<BaseFloat> patch1(patches, 0, out_deriv.NumRows(), 
                                          0, batch_size - index);
            ...
            if(batch_size - index < rem) {
                //KALDI_LOG << "HERE";
                end_frame +=  1;
                first_frame += 1;
            }

            //KALDI_LOG << "MIDDLE: " << end_frame * batch_size;
            for(int32 frame = 0; frame < end_frame; frame ++) {

                int32 offset = (frame + 1) * batch_size - index;
                //KALDI_LOG << "middle patches COLS: " << offset << "   " << out_deriv.NumCols();
                //KALDI_LOG << "middle patches ROWS: " << (frame + 1) << "   " << out_deriv.NumRows()-(frame + 1);


                CuSubMatrix<BaseFloat> data(out_deriv, (frame + 1), out_deriv.NumRows()-(frame + 1),
                                              0, out_deriv.NumCols());
                CuSubMatrix<BaseFloat> patch(patches, 0, out_deriv.NumRows()-(frame + 1), 
                                              offset, out_deriv.NumCols());

                patch.CopyFromMat(data);
            }


            int32 num_cols = rem + index - first_frame * batch_size;
            int32 offset =  (1 + end_frame) * batch_size - index;

            if(num_cols != 0) {

                //KALDI_LOG << "POST patch ROW: " << offset << "   " << num_cols;
                //KALDI_LOG << "POST patch COLS: " << (end_frame + 1) << "   " << out_deriv.NumRows()-(end_frame + 1);

                CuSubMatrix<BaseFloat> data2(out_deriv, (end_frame + 1), out_deriv.NumRows()-(end_frame + 1),
                                              0, num_cols);
                CuSubMatrix<BaseFloat> patch2(patches, 0, out_deriv.NumRows()-(end_frame + 1), 
                                              offset, num_cols);
    ...
    /*const int32 r = 10;
        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "OUTDA-1: r: " << r-1 << "c: " << out_value.NumCols() - 6 + c << "    " << out_value(r-1, out_value.NumCols() - 6 + c);
        }
        KALDI_LOG << " ";

        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "OUTDA: r: " << r << "c: " << c << "    " << out_value(r, c);
        }
        KALDI_LOG << " ";

        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "IN-1: r: " << r-1 << "c: " << in_value.NumCols() - 6 + c << "    " << in_value(r-1, in_value.NumCols() - 6 + c);
        }
        KALDI_LOG << " ";

        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "INDAT: r: " << r << "c: " << c << "    " << in_value(r,c);
        }

        KALDI_LOG << "----------------------------------------------------------------------------";
        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "OUTDA-1 dev: r: " << r-1 << "c: " << out_deriv.NumCols() - 6 + c << "    " << out_deriv(r-1, out_deriv.NumCols() - 6 + c);
        }
        KALDI_LOG << " ";

        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "OUTDA dev: r: " << r << "c: " << c << "    " << out_deriv(r, c);
        }
        KALDI_LOG << " ";

        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "IN-1 dev: r: " << r-1 << "c: " << out_deriv.NumCols() - 6 + c << "    " << (*in_deriv)(r-1, out_deriv.NumCols() - 6 + c);
        }
        KALDI_LOG << " ";

        for(int c = 0; c < 6; c++) {
            KALDI_LOG << "INDAT dev: r: " << r << "c: " << c << "    " << (*in_deriv)(r,c);
        }
...
Component *ConvRIRComponent::Copy() const {
  ConvRIRComponent *ans = new ConvRIRComponent();
  ans->input_dim_ = input_dim_;
  ans->filter_params_ = filter_params_;
  return ans;
}

void ConvRIRComponent::Read(std::istream &is, bool binary) {
ExpectOneOrTwoTokens(is, binary, "<ConvRIRComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "<RIRcoefficients>");
  filter_params_.Read(is, binary);
  ExpectToken(is, binary, "</ConvRIRComponent>");
  
  KALDI_LOG << "INPUT DIM: " << input_dim_;
  KALDI_LOG << "FILTER DIM: " <<  filter_params_.NumCols();;

  invers_filter_params_.Resize(filter_params_.NumRows(), filter_params_.NumCols());
  for(int c = 0; c < filter_params_.NumCols(); c++) {
    for(int r = 0; r <  filter_params_.NumRows(); r++) {
        invers_filter_params_(filter_params_.NumRows() - r - 1, filter_params_.NumCols() - c - 1) = filter_params_(r,c);
        //KALDI_LOG << "FILTER PARAM: " << filter_params_(r,c);
    }
  }

  /*for(int c = 0; c < invers_filter_params_.NumCols(); c++) {
    for(int r = 0; r <  invers_filter_params_.NumRows(); r++) {
        KALDI_LOG << "invers FILTER PARAM: " << invers_filter_params_(r,c);
    }
  }*/
}

void ConvRIRComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<ConvRIRComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "<RIRcoefficients>");
  filter_params_.Write(os, binary);
  WriteToken(os, binary, "</ConvRIRComponent>");
}


std::string OverlapComponent::Info() const {
  std::stringstream stream;
  stream << ", overlap=" << overlap_;

  return stream.str();
}

void OverlapComponent::Init(int32 input_dim, int32 overlap) {
...
void OverlapComponent::InitFromString(std::string args) {
  std::string orig_args(args);

  int32 input_dim, overlap;

  ParseFromString("input-dim", &args, &input_dim);
  ParseFromString("overlap", &args, &overlap);

  Init(input_dim, overlap);
}

int32 OverlapComponent::OutputDim() const {
  return input_dim_  + overlap_;
}

void OverlapComponent::Propagate(const ChunkInfo &in_info,
                                const ChunkInfo &out_info,
                                const CuMatrixBase<BaseFloat> &in,
                                CuMatrixBase<BaseFloat> *out) const  {

  CuSubMatrix<BaseFloat> in_part1(in, 0, in.NumRows()-1,
                                 0, in.NumCols()),
                         in_part2(in, 1, in.NumRows()-1,
                                 0, overlap_),
                         out_part1(*out, 0, out->NumRows()-1,
                                  0, in.NumCols()),
                         out_part2(*out, 0, out->NumRows()-1,
                                  in.NumCols(), overlap_);
  out_part1.CopyFromMat(in_part1);
  out_part2.CopyFromMat(in_part2);


  CuSubMatrix<BaseFloat> last_in1(in, in.NumRows()-1, 1,
                                 0, in.NumCols()),
                         last_in2(in, in.NumRows()-1, 1,
                                 in.NumCols() - overlap_, overlap_),
                         last_out1(*out, out->NumRows()-1, 1,
                                 0, in.NumCols()),
                         last_out2(*out, out->NumRows()-1, 1,
                                  in.NumCols(), overlap_);

  last_out1.CopyFromMat(last_in1);
  last_out2.CopyFromMat(last_in2);
}

void OverlapComponent::Backprop(const ChunkInfo &in_info,
                               const ChunkInfo &out_info,
                               const CuMatrixBase<BaseFloat> &in_value,
                               const CuMatrixBase<BaseFloat> &out_value,
                               const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  CuSubMatrix<BaseFloat>  in_part2(*in_deriv, 0, in_deriv->NumRows()-1,
                                 0, in_deriv->NumCols()),
                         out_part1(out_deriv, 0, out_deriv.NumRows()-1,
                                  0, in_deriv->NumCols());

  in_part2.CopyFromMat(out_part1);

  /*KALDI_LOG << "IN 1: " << in_value.Row(7);
  KALDI_LOG << "IN 2: " << in_value.Row(8);
  KALDI_LOG << "OUT: " << out_value.Row(7);

  KALDI_LOG << "IN DER: " << in_deriv->Row(7);

  KALDI_LOG << "OUT DER: " << out_deriv.Row(7);*/
}

Component *OverlapComponent::Copy() const {
  OverlapComponent *ans = new OverlapComponent();
  ans->input_dim_ = input_dim_;
  ans->overlap_ = overlap_;
  return ans;
}

void OverlapComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<OverlapComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "<Overlap>");
  ReadBasicType(is, binary, &overlap_);
  ExpectToken(is, binary, "</OverlapComponent>");
}

void OverlapComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<OverlapComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
...
void SpliceComponent::Backprop(const ChunkInfo &in_info,
  ...
  for (int32 chunk = 0; chunk < num_chunks; chunk++) {
    if (chunk == 0) { // this branch can be taken for all chunks, but is not
      ...
      for (int32 c = 0; c < num_splice; c++)  { //9
        for (int32 out_index = 0; out_index < out_chunk_size; out_index++) { // 208
          int32 out_offset = out_info.GetOffset(out_index);
          int32 in_index = in_info.GetIndex(out_offset + context_[c]); // 0 - 207
  ...
  for (int32 c = 0; c < num_splice; c++) { // 9
    CuArray<int32> cu_indexes(indexes[c]);
    int32 dim = input_dim - const_dim;  // 13 - 0 = 13 dimension we
    // are splicing

    CuSubMatrix<BaseFloat> out_deriv_part(out_deriv, 0, out_deriv.NumRows(), // 208
                                          c * dim, dim),
        in_deriv_part(*in_deriv, 0, in_deriv->NumRows(), // 216
                      0, dim);
...
std::string FftComponent::Info() const {
  std::stringstream stream;
  stream << Component::Info() << ", fft_dim=" << fft_mat_.NumCols();
  if (fft_mat_.NumCols() != fft_mat_.NumRows())
    stream << ", fft_keep_dim=" << fft_mat_.NumRows();

  return stream.str();
}

void FftComponent::Init(int32 dim, int32 fft_dim, bool reorder, int32 fft_keep_dim) {
  int fft_keep_dim_ = (fft_keep_dim > 0) ? fft_keep_dim : fft_dim;

  KALDI_ASSERT(dim > 0 && fft_dim > 0);
  KALDI_ASSERT(dim % fft_dim == 0); // fft_dim must divide dim.
  KALDI_ASSERT(fft_dim >= fft_keep_dim_);
  dim_ = dim;
  fft_mat_.Resize(fft_keep_dim_, fft_dim);
  reorder_ = reorder;
  Matrix<BaseFloat> fft_mat(fft_keep_dim_, fft_dim);
  ComputeFftMatrix(&fft_mat);

  fft_mat_ = fft_mat;
}

void FftComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  int32 dim, fft_dim, fft_keep_dim = 0;
  bool reorder = false;

  bool ok = ParseFromString("dim", &args, &dim);
  ok = ParseFromString("fft-dim", &args, &fft_dim) && ok;
  ok = ParseFromString("reorder", &args, &reorder) && ok;
  ParseFromString("fft-keep-dim", &args, &fft_keep_dim);

  if (!ok || !args.empty() || dim <= 0 || fft_dim <= 0 || fft_keep_dim < 0)
    KALDI_ERR << "Invalid initializer for layer of type "
              << Type() << ": \"" << orig_args << "\"";
  Init(dim, fft_dim, reorder, fft_keep_dim);
}

void FftComponent::Reorder(CuMatrixBase<BaseFloat> *mat, bool reverse) const {
  // reorders into contiguous blocks of dize "fft_dim_", assuming that
  // such blocks were interlaced before.  if reverse==true, does the
  // reverse.
  int32 fft_dim = fft_mat_.NumCols(),
      fft_keep_dim = fft_mat_.NumRows(),
      block_size_in = dim_ / fft_dim,
      block_size_out = fft_keep_dim;
...
void FftComponent::Propagate(const ChunkInfo &in_info,
                             const ChunkInfo &out_info,
                             const CuMatrixBase<BaseFloat> &in,
                             CuMatrixBase<BaseFloat> *out) const  {
  KALDI_ASSERT(in.NumCols() == InputDim());
  int32 fft_dim = fft_mat_.NumCols(),
        fft_keep_dim = fft_mat_.NumRows(),
        num_rows = in.NumRows(),
        num_chunks = dim_ / fft_dim;

  in_info.CheckSize(in);
  out_info.CheckSize(*out);
  KALDI_ASSERT(num_rows == out_info.NumRows());
  KALDI_ASSERT(num_chunks * fft_keep_dim == out_info.NumCols());
  ...
  for (int32 chunk = 0; chunk < num_chunks; chunk++) {
    CuSubMatrix<BaseFloat> in_mat(reorder_ ? in_tmp : in,
                                0, num_rows, fft_dim * chunk, fft_dim),
                        out_mat(*out,
                                0, num_rows, fft_keep_dim * chunk, fft_keep_dim);

    out_mat.AddMatMat(1.0, in_mat, kNoTrans, fft_mat_, kNoTrans, 0.0);
  }
  if (reorder_)
    Reorder(out, true);
}

void FftComponent::Backprop(const ChunkInfo &,  //in_info,
                            const ChunkInfo &,  //out_info,
                            const CuMatrixBase<BaseFloat> &,  //in_value,
                            const CuMatrixBase<BaseFloat> &,  //out_value,
                            const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  for (int32 chunk = 0; chunk < num_chunks; chunk++) {
    ...
    // Note: in the reverse direction the FFT matrix is transposed.  This is
    // normal when computing derivatives; the necessity for the transpose is
    // obvious if you consider what happens when the input and output dims
    // differ.
    in_deriv_mat.AddMatMat(1.0, out_deriv_mat, kNoTrans,
  ...
  /*CuVector<BaseFloat> tmp(fft_mat_.NumCols());
  for (int32 k = 0; k < fft_mat_.NumRows()/2; k++) {
    tmp(k) = 0;
    tmp(k+fft_mat_.NumCols()/2) = 0;
    for (int32 n = 0; n < fft_mat_.NumCols()/2; n++) {
      tmp(k) += fft_mat_(k,n);
      tmp(k+fft_mat_.NumCols()/2) += fft_mat_(k,n+fft_mat_.NumCols()/2);
    }
  }

  in_deriv->CopyFromMat(out_deriv);
  in_deriv->MulColsVec(tmp);*/

  if (reorder_)
    Reorder(in_deriv, true);
}

Component* FftComponent::Copy() const {
  FftComponent *ans = new FftComponent();
  ans->fft_mat_ = fft_mat_;
  ans->dim_ = dim_;
  ans->reorder_ = reorder_;
  return ans;
}

void FftComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<FftComponent>");
  WriteToken(os, binary, "<Dim>");
  WriteBasicType(os, binary, dim_);
  WriteToken(os, binary, "<FftDim>");
  int32 fft_dim = fft_mat_.NumCols();
  WriteBasicType(os, binary, fft_dim);
  WriteToken(os, binary, "<Reorder>");
  WriteBasicType(os, binary, reorder_);
  WriteToken(os, binary, "<FftKeepDim>");
  int32 fft_keep_dim = fft_mat_.NumRows();
  WriteBasicType(os, binary, fft_keep_dim);
  WriteToken(os, binary, "</FftComponent>");
}

void FftComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<FftComponent>", "<Dim>");
  ReadBasicType(is, binary, &dim_);

  ExpectToken(is, binary, "<FftDim>");
  int32 fft_dim;
  ReadBasicType(is, binary, &fft_dim);

  ExpectToken(is, binary, "<Reorder>");
  ReadBasicType(is, binary, &reorder_);

  int32 fft_keep_dim = fft_dim;
  std::string token;
  ReadToken(is, binary, &token);
  if (token == "<FftKeepDim>") {
    ReadBasicType(is, binary, &fft_keep_dim);
    ExpectToken(is, binary, "</FftComponent>");
  } else if (token != "</FftComponent>") {
    KALDI_ERR << "Expected token \"</FftComponent>\", got instead \""
              << token << "\".";
  }

  KALDI_ASSERT(fft_dim > 0 && dim_ > 0 && dim_ % fft_dim == 0);
  Init(dim_, fft_dim, reorder_, fft_keep_dim);
  //ifft_mat_.Resize(fft_keep_dim, fft_dim);
  //ComputeFftMatrix(&fft_mat_);
}
